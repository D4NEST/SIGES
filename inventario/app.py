...
from security import (
    security_manager, require_auth, require_admin, rate_limit, 
    validate_input, is_valid_string, is_valid_sku, is_valid_serial, is_valid_estado
)
...
def protected_route(f):
    """Decorator para rutas que requieren autenticación - MEJORADO"""
...
@validate_input(
    username=lambda x: is_valid_string(x, 1, 50),
    password=lambda x: is_valid_string(x, 1, 100)
)
def login():
    """Login seguro con protección contra ataques"""
    ...
    if security_manager.is_ip_locked(client_ip):
        security_manager.log_security_event(
            "LOGIN_BLOCKED", 
            f"IP bloqueada por intentos fallidos",
            client_ip
        )
        return jsonify({"error": "IP bloqueada temporalmente por intentos fallidos"}), 429
    ...
    try:
        ...
        username = security_manager.sanitize_input(data.get('username', '').strip())
        ...
        if username == admin_username and password == admin_password:
            ...
            session['session_token'] = security_manager.generate_session_token()
            ...
            security_manager.log_security_event(
                "LOGIN_FAILED", 
                f"Usuario: {username} - Credenciales inválidas",
                client_ip
            )
...
# ====================================================================
# API: GENERAR REPORTE COMPLETO PARA IMPRESIÓN
# ====================================================================
@app.route('/api/inventario/reporte/completo', methods=['GET', 'OPTIONS'])
@protected_route
def generar_reporte_completo():
    """Genera reporte completo del inventario para impresión"""
    if request.method == 'OPTIONS':
        return jsonify({}), 200
    
    conn = None
    try:
        print("🖨️ Iniciando generación de reporte...")
        ...
        # Obtener información completa del inventario
        query = """
        SELECT 
            p.producto_id,
            p.nombre,
            ...
            -- Seriales
            STRING_AGG(
                CASE WHEN s.codigo_unico_serial IS NOT NULL 
                THEN s.codigo_unico_serial || ' (' || s.estado || ')' 
                ELSE NULL END, 
                ', '
            ) as seriales_detalle
        FROM productos p
        ...
        GROUP BY 
            p.producto_id, p.nombre, p.marca, p.modelo, 
            tp.tipo_modelo, p.codigo_sku, p.descripcion
        ORDER BY p.marca, p.modelo, p.nombre;
        """
        
        print("📊 Ejecutando query de productos...")
        cur.execute(query)
        productos = [dict(row) for row in cur.fetchall()]
        print(f"✅ Productos obtenidos: {len(productos)}")
        
        # Estadísticas generales
        print("📈 Ejecutando query de estadísticas...")
        cur.execute("""
            SELECT 
                COUNT(DISTINCT p.producto_id) as total_productos,
                COUNT(DISTINCT s.serial_id) as total_seriales,
                SUM(CASE WHEN s.estado = 'ALMACEN' THEN 1 ELSE 0 END) as total_almacen,
                SUM(CASE WHEN s.estado = 'INSTALADO' THEN 1 ELSE 0 END) as total_instalado,
                SUM(CASE WHEN s.estado = 'DAÑADO' THEN 1 ELSE 0 END) as total_danado,
                SUM(CASE WHEN s.estado = 'RETIRADO' THEN 1 ELSE 0 END) as total_retirado
            FROM productos p
            LEFT JOIN seriales s ON p.producto_id = s.producto_id;
        """)
        
        estadisticas = dict(cur.fetchone())
        ...
        print(f"🎉 Reporte generado exitosamente - {len(productos)} productos")
