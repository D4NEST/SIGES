...
- **Tabla `votos_personal`**: Tabla principal de la base de datos donde se almacenan los registros procesados del personal.
...
- **Cruce de Cédulas**: Proceso de JOIN entre `votos_personal` y los datos del Archivo de Votos usando el campo `cedula` como llave foránea lógica.
...
3. WHEN el archivo es aceptado, THE Sistema SHALL leer los encabezados de la primera fila y presentar al operador el Wizard de Mapeo para asociar cada columna del Excel a un campo de `votos_personal`.
...
6. WHEN el operador confirma el mapeo, THE Sistema SHALL validar que el tipo de dato de cada columna mapeada sea compatible con el tipo del campo destino en `votos_personal` antes de despachar el Job de Importación.
...
3. WHEN un chunk de 500 filas se procesa exitosamente, THE Sistema SHALL realizar un `upsert` basado en el campo `cedula` para evitar duplicados en `votos_personal`.
...
1. WHEN el supervisor inicia el cruce de datos, THE Sistema SHALL realizar un `LEFT JOIN` entre `votos_personal` y la tabla de votos usando el campo `cedula` como llave de enlace.
...
4. IF una cédula existe en el archivo de votos pero no en `votos_personal`, THEN THE Sistema SHALL registrar esa cédula en un reporte de anomalías sin insertarla en la tabla principal.
...
1. WHEN un supervisor o administrador accede al dashboard, THE Sistema SHALL ejecutar consultas Eloquent con índices optimizados sobre `votos_personal` y devolver los resultados en menos de 5 segundos para conjuntos de hasta 100,000 registros.
...
4. WHEN el dashboard carga, THE Sistema SHALL mostrar la distribución de registros por `estado_fisico` geográfico.
...
1. WHEN el administrador ejecuta la función de archivado, THE Sistema SHALL exportar todos los registros de `votos_personal` con `created_at` anterior a 365 días en un archivo comprimido en formato `.zip` que contenga un archivo `.csv` con todos los campos, almacenado en `storage/app/private/archivos/`.
2. WHEN la exportación del archivo comprimido se complete exitosamente, THE Sistema SHALL eliminar de la tabla `votos_personal` únicamente los registros incluidos en ese archivo, utilizando una transacción de base de datos para garantizar la atomicidad de la operación.
